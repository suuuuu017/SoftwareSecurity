...
def save_mem_reg_state(uc):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(str(uc.reg_read(UC_X86_REG_EAX)));
        f.write(str(uc.reg_read(UC_X86_REG_ECX)));
        f.write(str(uc.reg_read(UC_X86_REG_EDX)));
        f.write(str(uc.reg_read(UC_X86_REG_EBX)));
        f.write(str(uc.reg_read(UC_X86_REG_ESP)));
        f.write(str(uc.reg_read(UC_X86_REG_EBP)));
        f.write(str(uc.reg_read(UC_X86_REG_ESI)));
        f.write('\n');
        pass
...
def load_mem_reg_state(uc):
    ...
    with open(filename, 'r') as f:
        ...
        for ln in lines:
            ...
            if i == 1:
                uc.reg_write(UC_X86_REG_ECX, int(ln))
            if i == 2:
            ...
            if i == 5:
                uc.reg_write(UC_X86_REG_EBP, int(ln))
            if i == 6:
                uc.reg_write(UC_X86_REG_ESI, int(ln))
            i = i + 1
...
def hook_code(uc, address, size, user_data):
    ...
    global cnt_repeated
    global valid_address_begin, valid_address_end

    trace_reg_changes(uc)

    print_instruction(uc, address, size)

    # if address > get_validaddr_end():
    #   valid_address_end = address
    #  set_validaddr_range(valid_address_begin, valid_address_end)
    # uc.emu_stop()
    # return
    if is_validaddr(address) == False:
        ...
        if address in cnt_repeated:
            ...
            output = "covering  addr %x (repeated: %d)" % (address, cnt_repeated[address])
            out(output)
            # if address > get_validaddr_begin():
            #   valid_address_begin = address
        else:
...
def hook_mem_invalid(uc, access, address, size, value, user_data):
    ...
    output = ("\t<!> Invalid Memory Access (%s) (stop analysis): (PC: %x), MemAddr %x, Size: %x, Value: %x" % (
    ac, pc, address, size, value))
    out(output)
    uc.emu_stop()
    Finish(uc)
    quit()
...
def hook_interrupt(uc, intno, user_data):
    ...
    ##############################
        ...
        output = ("[SYSCALL(INT 0x80)] SETUID")
        out(output)
    elif eax == 0:
        output = ("[SYSCALL(INT 0x80)] sys_restert_syscall")
        out(output)
    else:
...
def run_i386(mode, code):
    ...
    try:
        ...
        uc.hook_add(UC_HOOK_MEM_READ_UNMAPPED | UC_HOOK_MEM_WRITE_UNMAPPED | UC_HOOK_MEM_FETCH_INVALID,
                    hook_mem_invalid)
...
if __name__ == '__main__':
    ...
    run_i386(UC_MODE_32, CODE_EXAMPLE4)
